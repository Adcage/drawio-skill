...
- Provider 固定为本地链路：`local-cli -> user-node(可选显式配置)`
- 禁止远程导出（不上传 `.drawio` 数据）
- 默认参数：
  - `formats=png,svg`
  - `png_scale=4`
  - `provider=auto`
...
- 预检：`python scripts/drawio_preflight.py`
- 引导安装：`python scripts/drawio_bootstrap.py --non-admin`
- 导出：`python scripts/drawio_export.py --input <file.drawio> --output-dir outputs --formats png,svg --png-scale 4`
- 自检：`python scripts/drawio_layout_check.py --input <file.drawio>`
